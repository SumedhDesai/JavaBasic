...
public class TotalSumontheBoundariesandDiagonals {
	...
	public static void totalSum(int[][] mat) {
		...
		int n= mat.length;
		int totalSum= firstDigonalSum(mat,n)+secondDigonalSum(mat,n)+boundarySum(mat,n);
		if(n%2!=0) {
	...
	private static int firstDigonalSum(int[][] mat, int n) {
		int sum=0;
	...
	private static int secondDigonalSum(int[][] mat, int n) {
		int sum=0;
	...
	private static int boundarySum(int[][] mat, int n) {
		int sum=0;
		for(int i=1; i<n-1; i++) {
		sum+=mat[0][i];
		sum+=mat[n-1][i];
